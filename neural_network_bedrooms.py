...
def save_model(model, performance_metrics, config):
    if not isinstance(model, torch.nn.Module):
        ...
        folder = os.path.join('neural_networks/regression_bedrooms', datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
        try:
            os.makedirs(folder)
        except OSError as e:
            print(e)
...
def find_best_nn():
    ...
    print(best_model_fp)
    path = (f'/Users/apple/Documents/GitHub/Data_Science_Airbnb/neural_networks/regression_bedrooms/{best_model_fp}/')
...
def get_file_list():
    directory = '/Users/apple/Documents/GitHub/Data_Science_Airbnb/neural_networks/regression_bedrooms'
